...
// Based on base::win::registry but only implements a small fraction of it.
class registry {
public:
  ...
  LONG CreateWithDisposition(HKEY rootkey, const wchar_t *subkey, DWORD *disposition,
                             REGSAM access) {
    HKEY subhkey = NULL;
...
version_alias MajorMinorBuildToVersion(int major, int minor, int build) {
  if ((major == 5) && (minor > 0)) {
...
bool InternalRtlGetNtVersionNumbers(OSVERSIONINFOW *osvi) {
  if (osvi == nullptr)
    return false;
...
os_info::os_info()
  ...
  InternalRtlGetNtVersionNumbers(reinterpret_cast<OSVERSIONINFOW *>(&version_info));
  ...
  number_.patch = GetUBR();
  version_ = MajorMinorBuildToVersion(number_.major, number_.minor, number_.build);
